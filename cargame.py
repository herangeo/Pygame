...
class Jet:
    ...
    def move(self):
        self.x -= self.vel
        if self.x < -100:  # Reset position if off-screen
            self.x = WIDTH
...
class Bomb:
    def __init__(self, x, y):
        ...
        self.y = y
        self.vel = 10
        self.exploded = False  # Flag to track if bomb has exploded
        self.explosion_timer = 0  # Timer for explosion display
        self.house_hit = None  # House that was hit by the bomb
...
def main():
    ...
    run = True
    jet = Jet(WIDTH, 200)  # Start the jet off-screen
    bombs = []
    houses = [House(i * 150, HEIGHT - 150) for i in range(6)]  # Align houses properly
